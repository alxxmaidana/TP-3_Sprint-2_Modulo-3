...
export async function agregarNuevoSuperheroeController(_req, res) {
    try {
        const nuevoSuperheroe = new Superhero({
            ...
            nombreReal: "Dr. Robert Bruce Banner",
            planetaOrigen: "Tierra",
            poderes: [
                "Fuerza física ilimitada que aumenta con su ira",
                "Resistencia casi indestructible",
                "Saltar kilómetros",
                "Emitir energía gamma",
                "Respirar bajo el agua",
                "Sobrevibir en el espacio",
            ],
            aliados: ["Thor"],
            enemigos: ["La Abominación", "Hulk Rojo"],
            debilidad: "Ira Incontrolable",
...
export async function eliminarSueperheroePorNombreController(req, res) {
    try {
        ...
        await eliminarSueperheroePorNombre(nombreSuperheroe);
    } catch (err) {
        res.status(500).send({
            mensaje: "Error al eliminar el sueperheroe",
            err: err.mensaje,
        });
    }
}
